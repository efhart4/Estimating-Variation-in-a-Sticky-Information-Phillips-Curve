...
#-------------------------------------------------------------------------------
    ...
    acceptance_fixed <- Fixed_Data[[8]]
    print(paste("acceptance reate of fixed model is: ", acceptance_fixed))
    ...
    mean_he_fixed <- apply(he_samples_fixed, 1, mean)
    sd_he_fixed <- apply(he_samples_fixed, 1, sd)

    
    mean_covariance_fixed  <- apply(covariance_samples_fixed , 1, mean)
    sd_covariance_fixed  <- apply(covariance_samples_fixed , 1, sd)
    ...
    # Measure Table Fixed 
    measure_table_fixed <- matrix(
      c(mean_alpha_fixed, mean_lambda_fixed, mean_hv_fixed, mean_he_fixed, mean_covariance_fixed,
        sd_alpha_fixed, sd_lambda_fixed, sd_hv_fixed,sd_he_fixed, sd_covariance_fixed),
      nrow = 2, ncol = 5, byrow = TRUE
    )
    
    # Assign row and column names
    rownames(measure_table_fixed) <- c("Mean", "SD")
    colnames(measure_table_fixed) <- c("Alpha", "Lambda", "HV",  "HE", "Covariance")
...
#-------------------------------------------------------------------------------
    ...
    timing <- TVP_Data[[14]]
    print(paste("acceptance reate of TVP model is: ", acceptance_rate_TVP))
    ...
    png("/Users/modelt/Documents/Research/Estimating Shifts in Attention /project steps 2/step 11/attentionbounds.png", width = 1200, height = 600)
    
    matplot(timing[1:186], plot_matrix[1:186, ],
            type = "l", xaxt = "n", xlab = "", ylab = "1 - lambda", 
            cex.main = 1.5, col = c("red","blue", "red"), lty = c(2, 1, 2))
    
    axis(1, timing, format(timing, "%b %Y"), cex.axis = 1)
    ...
    # Analyzing graph
    print(cbind("Timing" = head(format(timing, "%b %Y"), 5), "Mean" = head(lambda_filtered, 5)))
    print(cbind("Timing" = tail(format(timing, "%b %Y"), 5), "Mean" = tail(lambda_filtered, 5)))
    ...
    min_timing <- timing[which.min(lambda_filtered[1:186])]
    max_timing <- timing[which.max(lambda_filtered[1:186])]
    ...
    print(paste("Last date:", format(timing[186], "%b %Y")))
    print(paste("First date:", format(timing[1], "%b %Y")))
